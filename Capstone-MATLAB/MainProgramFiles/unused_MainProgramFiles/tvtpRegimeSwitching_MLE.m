function [Spec_Out p11 p22 p12 p21 var1 var2 var3 ar1 ar2 ar3 c1 c2 c3] = ...
		 tvtpRegimeSwitching_MLE(k,inf_data)

		disp('entered RegimeSwitching_MLE succesfully');
		addpath('../../m_files_tvtp'); % add 'm_Files' folder to the search path
		

		logRet=inf_data;  % load some Data.

		dep=logRet(:,1);                 	% Defining dependent variables in system (solving for two dependent variables)
		nLag=1;                             % Number of lags in system
		k=k									% Number of States
		constVect=ones(length(dep),1);
		px=[constVec];
		...
		disp('COMPLETED (tvtp)MS_VAR_FIT function');

		p11=Spec_Out.Coeff.p(1,1);
		p22=Spec_Out.Coeff.p(2,2);
		p12=Spec_Out.Coeff.p(2,1); %IMPORTANT (DEVELOP CONVENTION FOR p12 nad p21)
		...
		%intercepts for each regime (These are the mean inflation rates for the AR(1) model)
		...
		%variances for each regime error term
